	...
	sameTime([function (done) {
		...
		function computeChanges(source, target) {
			...
			function _lumaValuesForWindow(image, x, y, width, height, luminance) {
				...
				for (var cY = y; cY < maxY; ++cY) {
					for (var cX = x; cX < maxX; ++cX) {
						var cPixel = image.getPixel(cX, cY),
							res = (cPixel.r * lumVals.r + cPixel.g * lumVals.g + cPixel.b * lumVals.b) * cPixel.a;
				...
				for (var y = 0; y < height; y += windowSize) {
					for (var x = 0; x < width; x += windowSize) {
						...
						var windowWidth = Math.min(windowSize, width - x),
							...
							lumaValues1 = _lumaValuesForWindow(image1, x, y, windowWidth, windowHeight, luminance),
							lumaValues2 = _lumaValuesForWindow(image2, x, y, windowWidth, windowHeight, luminance),
